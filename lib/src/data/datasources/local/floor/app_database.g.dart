    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `articles_table` (`id` INTEGER, `source` TEXT, `author` TEXT, `title` TEXT, `description` TEXT, `url` TEXT, `urlToImage` TEXT, `publishedAt` TEXT, `content` TEXT, PRIMARY KEY (`url`))');
...
class _$ArticleDao extends ArticleDao {
  ...
  @override
  Future<void> removeById(String url) async {
    await _queryAdapter.queryNoReturn(
        'DELETE FROM articles_table WHERE url = ?1',
        arguments: [url]);
  }

  @override
  Future<void> insert(ArticleEntity article) async {
